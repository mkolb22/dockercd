...
Verify the controller through the authenticated CLI or API. For example:

```sh
dockercd app get my-app --server http://localhost:8080
dockercd app diff my-app --server http://localhost:8080
```

For browser monitoring, deploy the separate `dockercd-web` presentation
service; the controller does not serve `/ui/`. See the
[`dockercd-web` local-auth deployment example](../deploy/docker-compose.web-local-auth.example.yml)
and the [operator recovery runbook](operator-recovery-runbook.md).
