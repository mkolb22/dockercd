...
2. **Check existing specs** (`spec_list`, `spec_get`). If a spec covers the area you're modifying, use its contracts to validate your implementation. If no spec exists for a significant module, create one with `spec_save`.
...
2. **Save a checkpoint** (`checkpoint_save`) after completing significant milestones — not just when asked.
...
- **Check coverage** (`analyze_coverage`, `find_untested_files`). After tests pass, verify coverage.
- **Auto-repair on failure** (`run_tests_with_repair`, `self_debug`, `iterative_refine`). When tests fail, use repair tools to diagnose and fix.

### Knowledge Graph

- **Bridge AST into KG** (`kg_ingest_ast`) after `index_project`.
- **Query relationships** (`kg_query`, `kg_traverse`) before cross-cutting changes.
- **Build the graph incrementally** (`kg_entity`, `kg_relate`).
- **Detect communities** (`kg_community`) to reveal module boundaries and coupling.
...
4. Check/create specs (`spec_list`, `spec_save`)
...
- Run `checkpoint_restore` with `latest: true` to reload pending work
...
- `.claude/` — Dragonfly agents, skills, commands, hooks
- `data/` — Dragonfly state (state.db, memory.db) — gitignored
- `docs/` — Documentation (design.md is the engineering spec)
- `node_modules/` — Dragonfly plugin — gitignored
