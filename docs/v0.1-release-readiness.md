# v0.1 release readiness matrix

## How to use this matrix

This document converts the locked [v0.1 goal](../GOAL.md) into release gates.
...
| Standard Compose deployment | Implemented | Normal deployment, failed health, retry, and rollback evidence with blue-green disabled/absent from v0.1 config. |
...
| Code quality, refactoring, and performance review | In progress | Complete focused cleanup of every v0.1 release tranche; preserve behavior with regression tests; record independent review and hot-path bounded-work/performance evidence. No unresolved P0/P1 and no undocumented P2 finding. |
...
| Separate read-only Web monitoring | Deployed; owner evidence pending | Scoped controller-status/capacity evidence is deployed in both pairs with local-password sign-in routes. Record owner workflow/accessibility evidence and controller failure/restart evidence before release approval. |
| Minimal paired deployment | Candidate deployed; Signal untouched | Both candidate pairs pass controller health, unauthenticated scoped-endpoint denial, and Web sign-in-route checks. Signal remains running and healthy. Before a personal application cutover, verify Signal backup/restore, pin or pause the existing `main`-tracking controller, validate interactive login, and use a separate Docker daemon/host if development/personal security isolation is required. See [deployment evidence](quality-first-delivery.md#scoped-capacity-and-controller-evidence-deployment-2026-09-18). |
...
metrics, broad logs, and topology. A v0.1 operator needs evidence to recover,
not a second dashboard platform.

### Deployment strategy

**Recommendation:** ship in-place standard Compose reconciliation only. Mark
blue-green and adoption unsupported in v0.1 configuration and documentation,
