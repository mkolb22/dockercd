/**
 * Prompt population management: mutation, crossover, validation.
 * Claude Code provides the intelligence; this module manages state.
 */
...
/**
 * Build a mutation prompt for the orchestrating agent.
 */
export function buildMutationPrompt(variant: PromptVariant, config: MutationConfig): string {
...
/**
 * Build a crossover prompt for combining two parent variants.
 */
 ...
 */
export function crossover(content: string, _config: CrossoverConfig): PromptVariant {
  return {
    variant_id: "",
    created_at: new Date().toISOString(),
...
/**
 * Validate that mutated content has valid markdown structure.
 */
export function validateMutatedContent(content: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
